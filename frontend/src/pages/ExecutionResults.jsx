...
const DELETE_CONFIRM_TOAST = {
  ...
  text: { fontSize: 12, color: '#6b7280', marginTop: 4, wordBreak: 'break-word' },
  actions: { display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 10 },
  btn: { border: '1px solid transparent', borderRadius: 8, fontSize: 12, fontWeight: 600, padding: '5px 12px', cursor: 'pointer' },
  cancelBtn: { borderColor: '#e5e7eb', color: '#6b7280', background: 'transparent' },
...
function LatencyBadge({ ms }) {
  if (ms === null || ms === undefined) return <span style={{ color: '#9ca3af' }}>—</span>;
  const color = ms < 500 ? '#16a34a' : ms < 1500 ? '#d97706' : '#dc2626';
  return (
    <span style={{ color, fontFamily: "'JetBrains Mono',monospace", fontSize: 13, fontWeight: 600 }}>
...
function StatCard({ label, value, sub, subColor, valueColor }) {
  return (
    <div style={{ background: '#fff', borderRadius: 12, padding: '20px 24px', boxShadow: '0 1px 3px rgba(0,0,0,0.06)', border: '1px solid #e5e7eb' }}>
      <div style={{ fontSize: 11, fontWeight: 600, color: '#9ca3af', textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 8 }}>{label}</div>
      <div style={{ fontSize: 28, fontWeight: 800, color: valueColor || '#111827', lineHeight: 1 }}>{value}</div>
      {sub && <div style={{ fontSize: 12, color: subColor || '#6b7280', marginTop: 6, fontWeight: 500 }}>{sub}</div>}
    </div>
  );
}
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 200, background: 'rgba(0,0,0,0.35)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }} onClick={onClose}>
      <div style={{ background: '#fff', borderRadius: 16, width: 500, maxHeight: '82vh', display: 'flex', flexDirection: 'column', boxShadow: '0 20px 60px rgba(0,0,0,0.15)', border: '1px solid #e5e7eb' }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '16px 20px', borderBottom: '1px solid #f3f4f6' }}>
          ...
          <span style={{ fontSize: 14, fontWeight: 600, color: '#111827', flex: 1 }}>File Bug Tickets</span>
        ...
        <div style={{ padding: '16px 20px', overflowY: 'auto', flex: 1, display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div style={{ display: 'flex', gap: 8 }}>
            {[
              ...
              return (
                <button key={key} onClick={() => configured && setPlatform(key)} style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 8, padding: '10px 14px', borderRadius: 10, border: `1.5px solid ${active ? '#7c3aed' : '#e5e7eb'}`, background: active ? '#f5f3ff' : '#fafafa', color: active ? '#7c3aed' : '#6b7280', fontSize: 13, fontWeight: 500, cursor: configured ? 'pointer' : 'not-allowed', opacity: configured ? 1 : 0.45, flexWrap: 'wrap' }}>
                  {icon} {label}
                  {info && configured
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4, maxHeight: 200, overflowY: 'auto' }}>
              {failedResults.map((r, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '7px 12px', background: '#fafafa', border: '1px solid #f3f4f6', borderRadius: 8 }}>
                  ...
                  <span style={{ fontSize: 12, color: '#374151', flex: 1 }}>{r.payloadName}</span>
                  <span style={{ fontSize: 11, color: '#9ca3af', fontFamily: 'monospace' }}>{r._reportName}</span>
                  <span style={{ fontSize: 11, fontFamily: 'monospace', color: '#dc2626', fontWeight: 600 }}>{r.statusCode}</span>
                </div>
              ))}
              {failedResults.length === 0 && (
                <div style={{ padding: '12px', textAlign: 'center', color: '#9ca3af', fontSize: 13 }}>No failed results in selected reports</div>
              )}
            </div>
          </div>
          ...
          {created.length > 0 && (
            <div style={{ paddingTop: 10, borderTop: '1px solid #f3f4f6', display: 'flex', flexDirection: 'column', gap: 6 }}>
              {created.map((c, i) => (
                <a key={i} href={c.issueUrl} target="_blank" rel="noopener noreferrer" style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: '#7c3aed', fontFamily: 'monospace', textDecoration: 'none' }}>
          ...
          {!bugConfig?.github && !bugConfig?.jira && (
            <div style={{ padding: '10px 12px', background: '#fffbeb', border: '1px solid #fcd34d', borderRadius: 8, fontSize: 12, color: '#92400e', lineHeight: 1.6 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, padding: '14px 20px', borderTop: '1px solid #f3f4f6' }}>
          <button onClick={onClose} style={{ padding: '7px 16px', border: '1px solid #e5e7eb', borderRadius: 8, background: '#fff', color: '#6b7280', fontSize: 13, cursor: 'pointer' }}>Cancel</button>
          <button
            onClick={handleSubmit}
            disabled={submitting || (!bugConfig?.github && !bugConfig?.jira) || created.length > 0 || !failedResults.length}
            style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '7px 18px', background: '#ef4444', border: 'none', borderRadius: 8, color: '#fff', fontSize: 13, fontWeight: 600, cursor: 'pointer', opacity: (submitting || (!bugConfig?.github && !bugConfig?.jira) || created.length > 0 || !failedResults.length) ? 0.5 : 1 }}
          >
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 50, background: 'rgba(0,0,0,0.25)', display: 'flex', justifyContent: 'flex-end' }} onClick={onClose}>
      <div style={{ width: '72%', maxWidth: 960, height: '100%', background: '#fff', display: 'flex', flexDirection: 'column', boxShadow: '-4px 0 24px rgba(0,0,0,0.12)' }} onClick={e => e.stopPropagation()}>
        <div style={{ padding: '18px 28px', borderBottom: '1px solid #f3f4f6', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 }}>
          <div>
            <div style={{ fontWeight: 700, fontSize: 15, color: '#111827' }}>{report.configName}</div>
            <div style={{ fontSize: 11, color: '#9ca3af', marginTop: 2, fontFamily: "'JetBrains Mono',monospace" }}>{report.method} {report.url}</div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            {['all', 'pass', 'fail'].map(f => (
              <button key={f} onClick={() => setFilter(f)} style={{ padding: '5px 14px', borderRadius: 20, border: '1px solid', borderColor: filter === f ? '#7c3aed' : '#e5e7eb', background: filter === f ? '#7c3aed' : '#fff', color: filter === f ? '#fff' : '#6b7280', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
                {f === 'all' ? `All (${report.results?.length ?? 0})` : f === 'pass' ? '✓ Pass' : '✕ Fail'}
              </button>
            ))}
            <button onClick={onClose} style={{ width: 32, height: 32, borderRadius: 8, border: '1px solid #e5e7eb', background: '#fff', fontSize: 18, cursor: 'pointer', color: '#6b7280', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>×</button>
          </div>
        </div>
            ...
            <thead>
              <tr style={{ background: '#fafafa', position: 'sticky', top: 0, zIndex: 1 }}>
                {['Payload Name', 'Edge Case', 'Status', 'Latency', 'Result', 'Download'].map(h => (
                  <th key={h} style={{ padding: '10px 20px', textAlign: 'left', fontSize: 11, fontWeight: 700, color: '#9ca3af', textTransform: 'uppercase', letterSpacing: 0.6, borderBottom: '1px solid #f3f4f6' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {rows.map((r, i) => (
                <tr key={r._id ?? i} style={{ background: i % 2 === 0 ? '#fff' : '#fafafa', borderLeft: !r.passed ? '3px solid #ef4444' : '3px solid transparent' }}>
                  <td style={{ padding: '12px 20px' }}>
                    <div style={{ fontWeight: 600, fontSize: 13, color: '#111827' }}>{r.payloadName}</div>
                    <div style={{ fontSize: 11, color: '#9ca3af', fontFamily: "'JetBrains Mono',monospace", marginTop: 2 }}>{String(r.payloadId || i).slice(-8)}</div>
                  </td>
                  <td style={{ padding: '12px 20px' }}>
                    <span style={{ background: '#f3f4f6', borderRadius: 6, padding: '3px 8px', fontSize: 11, fontFamily: "'JetBrains Mono',monospace", color: '#374151' }}>{r.edgeCaseType || '—'}</span>
                  </td>
                  <td style={{ padding: '12px 20px' }}>
                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '3px 10px', borderRadius: 20, background: r.passed ? '#dcfce7' : '#fee2e2', color: r.passed ? '#16a34a' : '#dc2626', fontSize: 12, fontWeight: 600, fontFamily: "'JetBrains Mono',monospace" }}>
                  ...
                  <td style={{ padding: '12px 20px' }}>
                    <span style={{ fontWeight: 700, fontSize: 12, color: r.passed ? '#16a34a' : '#dc2626' }}>{r.passed ? '✓ PASS' : '✕ FAIL'}</span>
                  </td>
                  <td style={{ padding: '12px 20px' }}>
                    <button onClick={() => downloadRowAsJson(r, report.configName)} title="Download as JSON"
                      style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '4px 10px', borderRadius: 6, border: '1px solid #e5e7eb', background: '#fff', color: '#6b7280', fontSize: 11, fontWeight: 500, cursor: 'pointer' }}>
...
export default function ExecutionResults() {
  ...
  const toggleAll = () =>
    setSelected(selected.size === reports.length ? new Set() : new Set(reports.map(r => r._id)));
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f0f2f5', fontFamily: "'DM Sans','Segoe UI',sans-serif", padding: '32px 40px' }}>

      <div style={{ marginBottom: 28 }}>
        <h1 style={{ fontSize: 22, fontWeight: 700, color: '#111827', margin: 0, letterSpacing: -0.3 }}>Execution Results</h1>
        <p style={{ fontSize: 13, color: '#6b7280', marginTop: 4 }}>All API test runs. Click a row to inspect individual payload results.</p>
      </div>
      ...
      <div style={{ background: '#fff', borderRadius: 12, boxShadow: '0 1px 3px rgba(0,0,0,0.06)', border: '1px solid #e5e7eb', overflow: 'hidden' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 20px', borderBottom: '1px solid #f3f4f6' }}>
          <div style={{ fontWeight: 600, fontSize: 15, color: '#111827' }}>
            Run History
            {!loading && <span style={{ marginLeft: 8, fontSize: 13, fontWeight: 400, color: '#9ca3af' }}>({reports.length} runs)</span>}
          </div>
          {selected.size > 0 && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <span style={{ fontSize: 12, color: '#6b7280', marginRight: 4 }}>{selected.size} selected</span>
              <button onClick={handleBulkPDF}
                style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 14px', borderRadius: 8, border: '1px solid #e5e7eb', background: '#fff', color: '#374151', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}
                onMouseEnter={e => e.currentTarget.style.background = '#f9fafb'}
                onMouseLeave={e => e.currentTarget.style.background = '#fff'}>
                <Download size={13} /> Download PDF{selected.size > 1 ? 's' : ''}
              </button>
              <button onClick={() => setShowBug(true)}
                style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 14px', borderRadius: 8, border: '1px solid #fca5a5', background: '#fff', color: '#ef4444', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}
                onMouseEnter={e => e.currentTarget.style.background = '#fff1f1'}
                onMouseLeave={e => e.currentTarget.style.background = '#fff'}>
                <Bug size={13} /> Report Bug
              </button>
              <button onClick={() => setSelected(new Set())}
                style={{ padding: '6px 10px', borderRadius: 8, border: '1px solid #e5e7eb', background: '#fff', color: '#9ca3af', fontSize: 12, cursor: 'pointer' }}>
                Clear
              </button>
            </div>
          )}
        ...
        {loading && <div style={{ padding: 48, textAlign: 'center', color: '#9ca3af', fontSize: 14 }}>Loading reports…</div>}
        {error   && <div style={{ padding: 48, textAlign: 'center', color: '#dc2626', fontSize: 14 }}>{error}</div>}
        {!loading && !error && reports.length === 0 && (
          <div style={{ padding: 48, textAlign: 'center', color: '#9ca3af', fontSize: 14 }}>No reports yet. Run an API test first.</div>
        )}
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: '#fafafa' }}>
                <th style={{ padding: '10px 16px', borderBottom: '1px solid #f3f4f6', width: 44 }}>
                ...
                {['Config Name', 'Method / URL', 'Payloads', 'Passed', 'Failed', 'Avg Latency', 'Run At', ''].map(h => (
                  <th key={h} style={{ padding: '10px 16px', textAlign: 'left', fontSize: 11, fontWeight: 700, color: '#9ca3af', textTransform: 'uppercase', letterSpacing: 0.6, borderBottom: '1px solid #f3f4f6' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {reports.map((report, idx) => {
                ...
                return (
                  <tr key={report._id}
                    style={{ background: isSelected ? '#faf5ff' : idx % 2 === 0 ? '#fff' : '#fafafa', cursor: 'pointer', transition: 'background 0.1s' }}
                  ...
                  >
                    ...
                    <td style={{ padding: '14px 16px' }}>
                      <div style={{ fontWeight: 600, fontSize: 14, color: '#111827' }}>{report.configName}</div>
                      <div style={{ fontSize: 11, color: '#9ca3af', fontFamily: "'JetBrains Mono',monospace", marginTop: 2 }}>{String(report._id).slice(-8)}</div>
                    </td>
                    <td style={{ padding: '14px 16px' }}>
                      <span style={{ fontSize: 11, fontWeight: 700, color: '#7c3aed', background: '#f5f3ff', padding: '2px 7px', borderRadius: 4, fontFamily: "'JetBrains Mono',monospace", marginRight: 6 }}>{report.method}</span>
                      <span style={{ fontSize: 12, color: '#6b7280', fontFamily: "'JetBrains Mono',monospace" }}>{report.url}</span>
                    </td>
                    <td style={{ padding: '14px 16px', fontSize: 14, color: '#374151', fontWeight: 500 }}>{report.totalPayloads}</td>
                    <td style={{ padding: '14px 16px' }}><span style={{ color: '#16a34a', fontWeight: 700, fontSize: 14 }}>{report.passed}</span></td>
                    <td style={{ padding: '14px 16px' }}><span style={{ color: report.failed > 0 ? '#dc2626' : '#16a34a', fontWeight: 700, fontSize: 14 }}>{report.failed}</span></td>
                    <td style={{ padding: '14px 16px' }}>{avg !== null ? <LatencyBadge ms={avg} /> : '—'}</td>
                    <td style={{ padding: '14px 16px', fontSize: 12, color: '#6b7280' }}>{new Date(report.runAt).toLocaleString()}</td>
                    <td style={{ padding: '14px 16px' }} onClick={e => e.stopPropagation()}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                        <button onClick={() => downloadReportPDF(report)} title="Download report as PDF"
                          style={{ width: 30, height: 30, borderRadius: 6, border: '1px solid #e5e7eb', background: '#fff', color: '#6b7280', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                          onMouseEnter={e => e.currentTarget.style.background = '#f9fafb'}
                          onMouseLeave={e => e.currentTarget.style.background = '#fff'}>
                          <Download size={13} />
                        </button>
